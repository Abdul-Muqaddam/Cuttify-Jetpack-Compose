...
class LanguageScreenViewModel : ViewModel() {
    ...
    private val _state =
        MutableStateFlow(LanguageDetails(languages = allLanguages))
    ...
    init {
        ...
        val updated =
            allLanguages.map { it.copy(isSelected = it.shortCode == saved) }

        _state.update {
            it.copy(selectedLanguage = saved, languages = updated)
        }
    }
    ...
    fun onLanguageSelect(languageAbbr: String) {
        ...
        val updated =
            allLanguages.map { it.copy(isSelected = it.shortCode == languageAbbr) }
    ...
    fun onLanguageSearch(query: String) {
        val filtered =
            if (query.isBlank()) allLanguages
                ...
                it.languageName.contains(query, true) ||
                        it.nativeName.contains(query, true)
            }

        _state.update {
            it.copy(
                languages = filtered.map {
                    it.copy(isSelected = it.shortCode == _state.value.selectedLanguage)
                }
            )
        }
    }
